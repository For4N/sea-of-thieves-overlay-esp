...
{
	char ErrorMsg[125];
	sprintf(ErrorMsg, "ESP by ForaN! F9 выход, F8 дебаг-мод, F7 живность, F6 хитбоксы", tWindowName);
	MessageBox(0, ErrorMsg, "Инфа", MB_OK | MB_ICONERROR);
	while(true)
	{
		...
		{
			char ErrorMsg[125];
			sprintf(ErrorMsg, "Запусти %s !", tWindowName);
			MessageBox(0, ErrorMsg, "Ошибка - Не могу найти игру!", MB_OK | MB_ICONERROR);
			exit(1);
		}
		Sleep(100);
	}
